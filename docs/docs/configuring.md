...
The Video queue, Video posters, and Video watermark tabs expose the automated media workflows. Queue timing, URL subfolder, and watermark numeric settings use Craft autosuggest fields for values that can come from environment variables. The watermark path supports environment values and also suggests Yii aliases. Numeric environment variables must resolve to integers within the same limits shown by their literal values.
...
A working Craft queue runner is required. For field uploads that Craft first saves in temporary storage, Transcoder waits for the final move before pushing the media job. The queue job waits for ffmpeg to finish. When an attempt fails, Transcoder queues another attempt after `videoEncodeRetryDelaySeconds`, up to `videoEncodeMaxRetries` retries. Set the retry count to `0` to disable automatic retries; after the final attempt, the Craft queue job fails normally and remains available for manual retry.
